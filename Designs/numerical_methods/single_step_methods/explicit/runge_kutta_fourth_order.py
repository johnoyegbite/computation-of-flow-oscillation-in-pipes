...
# This module is built for a Runge-Kutta 2nd order differential equation
class RungeKutta(numerical_method.NumericalMethod):
...
#               
    ...
    def display(self, _type):
        if self.A1 and self.A2: # if we have two reservoirs
            method_vals = self.runge_kutta_solution()
            ...
            self.plot_single({self.pipe_label: method_vals[self.pipe_label]}, 
                              self.pipe_label, method_label=self.runge_kutta_label)
#            self.plot_single({self.reservoir_1_label: method_vals[self.reservoir_1_label]}, 
#                              self.reservoir_1_label, method_label=self.runge_kutta_label)
#            self.plot_single({self.reservoir_2_label: method_vals[self.reservoir_2_label]},
#                              self.reservoir_2_label, method_label=self.runge_kutta_label)
#            self.plot_multiple(method_vals, _type + " (all-values)", 
#                               method_label=self.runge_kutta_label)
#            
...
#    g = 32.2
#     
#    rk = RungeKutta(D, L, v, e, g)
#    rk.set_initial_conditions(t0, z0, dzdt, h, t)
...
#    rk.set_reservoir_param(D1, D2, length_minor_losses, f)
