# -*- coding: utf-8 -*-
"""
Created on Tue Sep 24 04:39:42 2019
...
class AdamsBashForth(numerical_method.NumericalMethod):
    # Assign given values of "Adams Bashforth"
    def __init__(self, D, L, v, e, g=9.81):
        numerical_method.NumericalMethod.__init__(self, D, L, v, e, g)
        self.precomputed_fns = {}
    ...
    def bash_forth_4th(self, tn, zn, Vn, tn_1, zn_1, Vn_1, 
        ...
        if (tn, zn, Vn) in self.precomputed_fns:
            fn = self.precomputed_fns[(tn, zn, Vn)][0]
            gn = self.precomputed_fns[(tn, zn, Vn)][1]
        else:
            fn = self.fn(tn, zn, Vn)
            gn = self.gn1(tn, zn, Vn)
            self.precomputed_fns[(tn, zn, Vn)] = fn, gn
            
        if (tn_1, zn_1, Vn_1) in self.precomputed_fns:
            fn_1 = self.precomputed_fns[(tn_1, zn_1, Vn_1)][0]
            gn_1 = self.precomputed_fns[(tn_1, zn_1, Vn_1)][1]
        else:
            fn_1 = self.fn(tn_1, zn_1, Vn_1)
            gn_1 = self.gn1(tn_1, zn_1, Vn_1)
            self.precomputed_fns[(tn_1, zn_1, Vn_1)] = fn_1, gn_1
            
        if (tn_2, zn_2, Vn_2) in self.precomputed_fns:
            fn_2 = self.precomputed_fns[(tn_2, zn_2, Vn_2)][0]
            gn_2 = self.precomputed_fns[(tn_2, zn_2, Vn_2)][1]
        else:
            fn_2 = self.fn(tn_2, zn_2, Vn_2)
            gn_2 = self.gn1(tn_2, zn_2, Vn_2)
            self.precomputed_fns[(tn_2, zn_2, Vn_2)] = fn_2, gn_2
            
        if (tn_3, zn_3, Vn_3) in self.precomputed_fns:
            fn_3 = self.precomputed_fns[(tn_3, zn_3, Vn_3)][0]
            gn_3 = self.precomputed_fns[(tn_3, zn_3, Vn_3)][1]
        else:
            fn_3 = self.fn(tn_3, zn_3, Vn_3)
            gn_3 = self.gn1(tn_3, zn_3, Vn_3)
            self.precomputed_fns[(tn_3, zn_3, Vn_3)] = fn_3, gn_3
    ...
    def bash_forth_solution(self):
        ...
        # note that the bashforth continuing values does not contain t0, z0 and V0
        # hence it has just the '3' values from the Runge-Kutta starting values.
        ab_continuing_values = []
        ab_continuing_values.extend(rk_starting_values)
        
        
        # Count number of iterations using step size or 
        # step height h.
        n = int((self.t - self.t0) / self.h)
        # note that 'i' ends at '4' from the first 'while loop iteration'
        # and we want to begin 'i' also at '4'.
        while i <= n:
            # Update the next value of z and V
            zn, Vn = self.bash_forth_4th(tn, zn, Vn, tn_1, zn_1, Vn_1,
                                         tn_2, zn_2, Vn_2, tn_3, zn_3, Vn_3)

            # Update the next value of t
            tn += h
            
            ab_continuing_values.append((tn, zn, Vn))

            # the first continuing values only has the Runge-kutta '3' starting
            # values.
            tn_3, zn_3, Vn_3 = ab_continuing_values[0]
            tn_2, zn_2, Vn_2 = ab_continuing_values[1]
            tn_1, zn_1, Vn_1 = ab_continuing_values[2]
            tn, zn, Vn = ab_continuing_values[3]
            
            # now, ab_continuing_values has four (4) values but
            # the last three (3) values would be picked for the next iteration.
            ab_continuing_values = ab_continuing_values[1:]
    ...
    def display(self, _type):
        if self.A1 and self.A2: # if we have two reservoirs
            method_vals = self.bash_forth_solution()
...
#    ab = AdamsBashForth(D, L, v, e, g)
#    ab.set_initial_conditions(t0, z0, dzdt, h, t)
...
#    ab.set_reservoir_param(D1, D2, length_minor_losses, f)
